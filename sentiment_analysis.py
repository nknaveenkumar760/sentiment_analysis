...
def preprocessing_data(data):
    ...
    for single_data in data:
        if len(single_data.split(",")) == 2 and single_data.split(",")[
                1] != "":
...
def simple_evaluation(evaluation_data):
    ...
    for index in range(0, total):
        analysis_result = analyse_text(
            classifier, vectorizer, evaluation_text[index])
...
accuracy = (true_positives + true_negatives) / (true_positives + \
            true_negatives + false_positives + false_negatives) * 100
