    ...
    for hdr, body in plx.plx_sections:
        if hdr.endswith("*"):
            if hdr.startswith("setup"):
                setattr(PlXTest, "setUp", partial(PlXTest._test_template, header=hdr[:-1], body=body))
            elif hdr.startswith("teardown"):
                setattr(PlXTest, "tearDown", partial(PlXTest._test_template, header=hdr[:-1], body=body))
